...
public class Google {
	...
	public void init(final OnInitCallBack oick) {
		...
		try {
			...
			GoogleApiClient.Builder builder = new GoogleApiClient.Builder(activity).addConnectionCallbacks(ConnectionCb)
					.addOnConnectionFailedListener(ConnectionFailCb).addApi(Games.API).addScope(Games.SCOPE_GAMES);
	...
	public void showAchievement() {
		activity.startActivityForResult(Games.Achievements.getAchievementsIntent(googleApiClient), REQUEST_CODE_GAMES);
	}
	...
	public void showLeaderboard() {
		activity.startActivityForResult(Games.Leaderboards.getAllLeaderboardsIntent(googleApiClient),
				REQUEST_CODE_GAMES);
	}

	public void shareLink(String contentUrl, String desc, OnLinkShareCallBack listener) {
	...
	// 只能分享本地图片
	// 必须安装Google+
	public void sharePhoto(Activity activity, String imageUri, String desc, OnPhotoShareCallBack listener) {
		try {
			if (GooglePlayServicesUtil.isGooglePlayServicesAvailable(activity) == ConnectionResult.SUCCESS) {
				...
				PlusShare.Builder share = new PlusShare.Builder(activity).setText(desc).addStream(ur)
						.setType("image/*");
				Intent intent = share.getIntent();
				activity.startActivityForResult(intent, REQUEST_CODE_INTERACTIVE_POST);
			} else {
				Log.d(TAG, "Google服务无法使用");
			}
		} catch (Exception e) {
	...
	public void consume(final String sku, String itemType, String signature, String purchaseOriginalData,
